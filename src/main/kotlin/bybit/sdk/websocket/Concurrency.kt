...
import bybit.sdk.websocket.wrapCrashHandlingUnit
...
object Concurrency {
    ...
    fun <T> runBlocking(
        ...
        return kotlinx.coroutines.runBlocking(addName(context, name)) {
            try {
                ...
                println("uncaught Throwable: ${ex.message}")
                null
            }
        }
    }
    ...
    fun runOnNonDaemonThreadPool(name: String? = null, block: suspend CoroutineScope.() -> Unit) = run(
        name, CoroutineScope(
...
fun CoroutineScope.launchCrashHandling(
    ...
    return launch(addName(context, name)) {
        try {
            ...
            // do nothing b/c it's probably just cancelling the awaitPing function
